...
guestList.forEach((listGuest) =>
  console.log(
    `Asslam o Alikum ${listGuest}, \n You are warmly invited for a dinner with me`
  )
);
...
guestList.forEach((newGuest) =>
  console.log(
    `Asslam o Alikum ${newGuest}, \n Would you like to dinner with me?`
  )
);

console.log(
  "Great news ! I found a bigger dinner table, so now I can invite more guests!"
);
...
console.log(
  "Unfortunately new dinner table won’t arrive in time for the dinner,so I can invite only two people for dinner"
);
...
while (guestList.length > 2) {
  ...
  console.log(
    `Sorry,  ${removeGuestName} I can't invite you for dinner today `
  );
}
